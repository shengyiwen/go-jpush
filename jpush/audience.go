...
type Audience struct {
	all             bool     // 如果all为true的话，则剩下标签无效
	tag             []string // 标签，多组标签的关系是Or的关系，即取并集
	tagAnd          []string // 标签，多组标签的关系是And关系，取交集
	tagNot          []string // 标签，多组标签的关系是先取并集，再取补集
	alias           []string // 别名，多个别名之间是Or的关系，取交集
	registrationIds []string // 注册ID，多个注册Id之间是Or的关系，即取并集
	segment         []string // 用户分群id，分群id
	abTest          []string // AB测Id，在页面创建AB测
}
